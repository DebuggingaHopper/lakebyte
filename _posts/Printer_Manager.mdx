...
In this article we will detail the main components that create the first version of the printer troubleshooter. This will not include any mention of prnport or the installer mentioned that helps with configruing the executables config file.
...
```c#
        ...
        readonly string resourceName = "LoccioniQueueManager.ClearQ.bat";
        readonly string temp_path = System.IO.Path.GetTempPath() + "\\LQM.bat";
        readonly string Printer = ConfigurationManager.AppSettings["PrinterName"];
```
...
```c#
            ...
            ExecuteCommand(temp_path, Printer);
...
```C#
        public void ExecuteCommand(string command, string printerName)
        {


            var processInfo = new ProcessStartInfo("cmd.exe", "/c " + command + " " + "\"" + printerName + "\"")
            {
                CreateNoWindow = true,
...
Honestly, the code to run the splash screen is not difficult, all we are doing is running it in its thread so it shows for 7 seconds and then after those 7 seconds, we create the script, run it, and then delete it. This code would be in the same location  but before where the assmebly is loaded and creates the temporary bat file.
```c#
